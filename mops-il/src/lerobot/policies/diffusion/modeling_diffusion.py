...
class DiffusionRgbEncoder(nn.Module):
    ...
    def __init__(self, config: DiffusionConfig, feature_key: str | None = None) -> None:
        super().__init__()
        ...
        if feature_key is not None:
            images_shape = config.image_features[feature_key].shape

        else:
            images_shape = next(iter(config.image_features.values())).shape
        num_channels = images_shape[0]
        ...
        # Handle input channels mismatch
        if num_channels != 3:
            if not hasattr(backbone_model, "conv1"):
                raise ValueError(
                    f"Backbone {config.vision_backbone} does not have a 'conv1' layer, "
                    f"so we cannot adapt it to {num_channels} channels."
                )
            ...
            # Initialize weights from pretrained backbone if available
            if config.pretrained_backbone_weights:
                with torch.no_grad():
                    if num_channels == 1:
                        # For 1 channel, average the RGB weights
                        new_conv.weight.data = first_conv.weight.data.mean(
                            dim=1, keepdim=True
                        )
                    # For other channels, we leave it as default initialization
                    # (which is usually a reasonable random init)
