...
def decode_video_frames_torchvision(
    ...
    closest_frames = closest_frames.type(torch.uint8)  # / 255
...
def get_video_pixel_channels(pix_fmt: str) -> int:
    if "gray" in pix_fmt or "depth" in pix_fmt or "monochrome" in pix_fmt:
        ...
        raise ValueError(f"Unknown format: {pix_fmt}")
...
def encode_lossless_video(
    input_dir: Path, output_path: Path, fps: int, overwrite: bool = False
) -> None:
    """Encodes a directory of images into a lossless video file.

    Args:
        input_dir (Path): Directory containing input images.
        output_path (Path): Path to save the output video file.
    ...
    # Use FFmpeg for lossless encoding
    input_pattern = str(
        input_dir / "frame-%06d.png"
    )  # Assuming images are named frame-000000.png, frame-000001.png, etc.
    command = [
        "ffmpeg",
        ...
        "-i",
        input_pattern,
        # --- Lossless RGB Settings ---
        "-c:v",
        "libx264rgb",  # The confirmed RGB-aware H.264 encoder
        "-crf",
        "0",  # Mathematically Lossless compression
        "-preset",
        "ultrafast",  # Fastest encoding, minimal compression optimization
        "-pix_fmt",
        "rgb24",  # Explicitly set the pixel format to RGB 24-bit
        # Note: The output file should be .mkv for best results
        str(output_path),
    ]
