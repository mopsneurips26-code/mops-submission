...
def convert_tasks(root, new_root) -> None:
...
def concat_data_files(paths_to_cat, new_root, chunk_idx, file_idx, image_keys) -> None:
    ...
    for ep_path in tqdm.tqdm(ep_paths, desc=f"convert videos of {video_key}"):
        ...
        if (
            ...
            for i, _ in enumerate(paths_to_cat):
                ...
                episodes_metadata[past_ep_idx][f"videos/{video_key}/chunk_index"] = (
                    chunk_idx
                )
                episodes_metadata[past_ep_idx][f"videos/{video_key}/file_index"] = (
                    file_idx
                )
...
def convert_info(root, new_root, data_file_size_in_mb, video_file_size_in_mb) -> None:
    info = load_info(root)
