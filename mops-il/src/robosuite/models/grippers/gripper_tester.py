...
class GripperTester:
    ...
    def __init__(
        ...
        if gripper_low_pos > gripper_high_pos:
            raise ValueError(
                f"gripper_low_pos {gripper_low_pos} is larger than gripper_high_pos {gripper_high_pos}"
            )
    ...
    def _apply_gripper_action(self, action) -> None:
    ...
    def loop(self, total_iters=1, test_y=False, y_baseline=0.01) -> None:
        ...
        for _cur_iter in range(total_iters):
            for cur_plan in seq:
                self.gripper_z_is_low, self.gripper_is_closed = cur_plan
                for _step in range(self.step_time):
                    self.step()
            if test_y and not self.object_height > y_baseline:
                raise ValueError(
                    f"object is lifed by {self.object_height}, "
                    + f"not reaching the requirement {y_baseline}"
                )
