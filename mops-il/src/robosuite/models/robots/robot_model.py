...
from robosuite.models.bases import (
    LegBaseModel,
    MobileBaseModel,
    MountModel,
    NullBaseModel,
...
def register_robot(target_class) -> None:
...
class RobotModel(MujocoXMLModel, metaclass=RobotModelMeta):
    ...
    def __init__(self, fname: str, idn=0) -> None:
        ...
        self.mujoco_model: mujoco.MjModel | None = None

    def set_mujoco_model(self, mujoco_model: mujoco.MjModel | None = None) -> None:
        if mujoco_model is not None:
            self.mujoco_model = mujoco_model
        else:
            self.mujoco_model = self.get_model()

    def set_base_xpos(self, pos: np.ndarray) -> None:
    ...
    def set_base_ori(self, rot: np.ndarray) -> None:
    ...
    def set_joint_attribute(self, attrib: str, values: np.ndarray, force=True) -> None:
        ...
        assert values.size == len(self._elements["joints"]), (
            ...
            + f"Values must be same size as joint dimension. Got {values.size}, expected {self.dof}!"
        )
    ...
    def add_base(self, base: RobotBaseModel) -> None:
    ...
    def add_mount(self, mount: MountModel) -> None:
        """Mounts @mount to arm.
    ...
    def add_mobile_base(self, mobile_base: MobileBaseModel) -> None:
        ...
        mount_support = find_elements(
            root=self.worldbody,
            tags="body",
            attribs={"name": mobile_base.correct_naming("support")},
            return_first=True,
        )
    ...
    def add_leg_base(self, leg_base: LegBaseModel) -> None:
        """Mounts @mobile_base to arm.
        ...
        arm_root = find_elements(root=self.worldbody, tags="body", return_first=False)[
            1
        ]
    ...
    def add_null_base(self, base: NullBaseModel) -> None:
