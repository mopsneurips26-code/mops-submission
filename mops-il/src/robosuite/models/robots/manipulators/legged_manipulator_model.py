...
class LeggedManipulatorModel(ManipulatorModel):
    ...
    def _remove_joint_actuation(self, part_name) -> None:
        ...
        for motor in self.actuator.findall(".//motor"):
            if part_name in motor.get("name"):
                ...
                self._actuators.remove(
                    motor.get("name").replace(self.naming_prefix, "")
                )
        ...
        for equality in self.equality.findall(".//joint"):
            ...
            if part_name in equality.get("joint1") or part_name in equality.get(
                "joint2"
            ):
