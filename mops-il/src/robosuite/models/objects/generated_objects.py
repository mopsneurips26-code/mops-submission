...
class CompositeBodyObject(MujocoGeneratedObject):
    ...
    def __init__(
        ...
        if isinstance(objects, MujocoObject):
            ...
            raise ValueError(f"Invalid objects received, got type: {type(objects)}")
    ...
    def _append_object(self, root, obj, parent_name=None, pos=None, quat=None) -> None:
        ...
        assert parent is not None, (
            f"Could not find parent body with name: {parent_name}"
        )
    ...
    def _append_joints(self, root, body_name=None, joint_specs="default") -> None:
        ...
        assert body is not None, f"Could not find body with name: {body_name}"
        # Add joint(s) to this body
        for joint_spec in joint_specs:
            body.append(new_joint(**joint_spec))

    @staticmethod
    def _remove_joints(body) -> None:
...
class PrimitiveObject(MujocoGeneratedObject):
    ...
    def __init__(
        ...
        elif isinstance(friction, (float, int)):
