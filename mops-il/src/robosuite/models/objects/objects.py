...
class MujocoObject(MujocoModel):
    ...
    def __init__(
        self, obj_type="all", duplicate_collision_geoms=True, scale=None
    ) -> None:
        super().__init__()
        self.asset = ET.Element("asset")
        assert obj_type in GEOM_GROUPS, (
            f"object type must be one in {GEOM_GROUPS}, got: {obj_type} instead."
        )
    ...
    def set_scale(self, scale, obj=None) -> None:
    ...
    def _get_object_subtree(self) -> Never:
    ...
    def _get_object_properties(self) -> None:
    ...
    def get_bounding_box_half_size(self) -> Never:
        raise NotImplementedError
...
class MujocoXMLObject(MujocoObject, MujocoXML):
    ...
    def bottom_offset(self):
        bottom_site = self.worldbody.find(
            f"./body/site[@name='{self.naming_prefix}bottom_site']"
        )
    ...
    def top_offset(self):
        top_site = self.worldbody.find(
            f"./body/site[@name='{self.naming_prefix}top_site']"
        )
    ...
    def horizontal_radius(self):
        horizontal_radius_site = self.worldbody.find(
            f"./body/site[@name='{self.naming_prefix}horizontal_radius_site']"
        )
    ...
    def _get_elements_by_name(self, geom_names, body_names=None, joint_names=None):
        ...
        """
        # names of every geom
        geoms = {geom_name: [] for geom_name in geom_names}
...
class MujocoGeneratedObject(MujocoObject):
    ...
    def __init__(self, obj_type="all", duplicate_collision_geoms=True) -> None:
        super().__init__(
    ...
    def append_material(self, material) -> None:
    ...
    def exclude_from_prefixing(self, inp) -> bool:
        ...
        return bool(inp in self.shared_materials or inp in self.shared_textures)
