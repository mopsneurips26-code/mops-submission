...
def register_env(target_class) -> None:
...
class MujocoEnv(metaclass=EnvMeta):
    ...
    def __init__(
        ...
        self.has_offscreen_renderer = (
            has_renderer and renderer != "mjviewer"
        ) or has_offscreen_renderer
    ...
    def initialize_renderer(self) -> None:
        ...
        if self.renderer == "mujoco":
            ...
            if self.render_camera is not None:
                assert len(self.render_camera) == 1, (
                    "Only one camera can be specified for mjviewer"
                )
                camera_id = self.sim.model.camera_name2id(self.render_camera[0])
            else:
                camera_id = None
            self.viewer = MjviewerRenderer(
                env=self, camera_id=camera_id, **self.renderer_config
            )
        else:
            raise ValueError(
    ...
    def initialize_time(self, control_freq) -> None:
        ...
        self.control_freq = control_freq
        if control_freq <= 0:
            raise SimulationError(f"Control frequency {control_freq} is invalid")
        self.control_timestep = 1.0 / control_freq

    def set_xml_processor(self, processor) -> None:
    ...
    def _initialize_sim(self, xml_string=None) -> None:
    ...
    def reset(self):
        ...
        self.visualize(vis_settings=dict.fromkeys(self._visualizations, False))
    ...
    def _reset_observables(self) -> None:
        if self.hard_reset:
    ...
    def _reset_internal(self) -> None:
        ...
        if self.has_offscreen_renderer:
            if self.sim._render_context_offscreen is None:
                MjRenderContextOffscreen(self.sim, device_id=self.render_gpu_device_id)
    ...
    def set_ep_meta(self, meta) -> None:
    ...
    def unset_ep_meta(self) -> None:
        """Unset episode meta data."""
        self._ep_meta = {}

    def _update_observables(self, force=False) -> None:
    ...
    def step(self, action):
        ...
        for _i in range(int(self.control_timestep / self.model_timestep)):
    ...
    def clear_objects(self, object_names) -> None:
    ...
    def reset_from_xml_string(self, xml_string) -> None:
    ...
    def add_observable(self, observable) -> None:
        ...
        assert observable.name not in self._observables, (
            f"Observable name {observable.name} is already associated with an existing observable! Use modify_observable(...) "
            "to modify a pre-existing observable."
        )
        self._observables[observable.name] = observable

    def modify_observable(self, observable_name, attribute, modifier) -> None:
        ...
        """
        # Find the observable
        assert observable_name in self._observables, (
            f"No valid observable with name {observable_name} found. Options are: {self.observation_names}"
        )
    ...
    def observation_modalities(self):
        ...
        """
        return {observable.modality for observable in self._observables.values()}
