...
class RobotEnv(MujocoEnv):
    ...
    def __init__(
        ...
        for i, camera_s in enumerate(self.camera_segmentations):
            if isinstance(camera_s, (list, tuple)):
                seg_is_nested = True
                break
    ...
    def _create_segementation_sensor(
        ...
        def camera_segmentation(obs_cache):
            ...
            if mapping is not None:
                seg = (
                    np.fromiter(
                        (mapping.get(x, -1) for x in seg.flatten()), dtype=np.int32
    ...
    def _pre_action(self, action, policy_step=False) -> None:
        ...
        cutoff = 0
        for _idx, robot in enumerate(self.robots):
    ...
    def _load_robots(self) -> None:
        ...
        for idx, (name, config) in enumerate(
            zip(self.robot_names, self.robot_configs, strict=False)
        ):
    ...
    def _check_robot_configuration(self, robots) -> Never:
