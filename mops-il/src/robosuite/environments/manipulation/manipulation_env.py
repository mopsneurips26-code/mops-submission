...
class ManipulationEnv(RobotEnv):
    ...
    def _get_world_pose_in_gripper_sensor(self, prefix, fn_name, modality):
        ...
        def fn(obs_cache):
            return (
                T.pose_inv(
                    T.pose2mat(
                        (obs_cache[f"{prefix}eef_pos"], obs_cache[f"{prefix}eef_quat"])
                    )
                )
    ...
    def _get_rel_obj_eef_sensor(
        self, prefix, obj_key, fn_name, new_key_prefix, modality
    ):
        """Creates a sensor function that returns the relative position between the object specified by @obj_key
        ...
        def fn(obs_cache):
            ...
            if any(
                ...
                for name in [
                    f"{obj_key}_pos",
                    f"{obj_key}_quat",
                    f"world_pose_in_{prefix}gripper",
                ]
            ):
                return np.zeros(3)
            obj_pose = T.pose2mat(
                (obs_cache[f"{obj_key}_pos"], obs_cache[f"{obj_key}_quat"])
            )
            rel_pose = T.pose_in_A_to_pose_in_B(
                obj_pose, obs_cache[f"world_pose_in_{prefix}gripper"]
            )
    ...
    def _check_grasp(self, gripper, object_geoms):
        ...
        if isinstance(gripper, GripperModel):
            ...
            assert all(isinstance(gripper[arm], GripperModel) for arm in gripper), (
        ...
        return all(self.check_contact(g_group, o_geoms) for g_group in g_geoms)
                ...
                return min(
                    [
                        self._gripper_to_target(
                            gripper[arm], target, target_type, return_distance
                        )
                        for arm in gripper
                    ]
                )
    ...
    def _visualize_gripper_to_target(self, gripper, target, target_type="body") -> None:
    ...
    def _check_robot_configuration(self, robots) -> None:
        ...
        for robot in robots:
            assert issubclass(ROBOT_CLASS_MAPPING[robot], FixedBaseRobot) or issubclass(
                ROBOT_CLASS_MAPPING[robot],
                MobileRobot or issubclass(ROBOT_CLASS_MAPPING[robot], LeggedRobot),
            ), (
