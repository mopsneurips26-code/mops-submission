...
class NutAssembly(ManipulationEnv):
    ...
    def __init__(
        ...
        if nut_type is not None:
            assert nut_type in self.nut_to_id, (
                f"invalid @nut_type argument - choose one of {list(self.nut_to_id.keys())}"
            )
            ...
            for i, nut in enumerate(active_nuts):
                ...
                for nut_name, idn in self.nut_to_id.items():
                    if nut_name in nut.name.lower():
                        ...
                        break
                if not valid_obj:
                    raise Exception(f"Got invalid object to reach: {nut.name}")
    ...
    def _load_model(self) -> None:
        ...
        if self.placement_initializer is None:
            ...
            for nut_name, default_y_range in zip(
                nut_names, ([0.11, 0.225], [-0.225, -0.11]), strict=False
            ):
        ...
        for _i, (nut_cls, nut_name) in enumerate(
            zip(
    ...
    def _setup_observables(self):
        ...
        if self.use_object_obs:
            ...
            # Create observables
            for name, s, enabled, active in zip(
                names, sensors, enableds, actives, strict=False
            ):
    ...
    def _create_nut_sensors(self, nut_name, modality="object"):
        ...
        sensors = [
            self._get_rel_obj_eef_sensor(
                arm_pf, nut_name, f"{nut_name}_to_{full_pf}eef_pos", full_pf, modality
            )
            for arm_pf, full_pf in zip(arm_prefixes, full_prefixes, strict=False)
        ]
        sensors += [
            self._get_obj_eef_rel_quat_sensor(
                full_pf, nut_name, f"{nut_name}_to_{full_pf}eef_quat", modality
            )
    ...
    def _check_success(self):
        ...
        for i, nut in enumerate(self.nuts):
            ...
            dist = min(
                [
                    np.linalg.norm(
                        self.sim.data.site_xpos[self.robots[0].eef_site_id[arm]]
                        - obj_pos
                    )
...
class NutAssemblySquare(NutAssembly):
    ...
    def __init__(self, **kwargs) -> None:
        assert "single_object_mode" not in kwargs and "nut_type" not in kwargs, (
