...
class TwoArmPegInHole(TwoArmEnv):
    ...
    def _load_model(self) -> None:
        ...
        hole_offset_fn = _OBJECT_POS_OFFSET_FN.get(
            hole_robot.robot_model.__class__.__name__, {}
        ).get("hole", lambda x: x)
        ...
        peg_pos_offset_fn = _OBJECT_POS_OFFSET_FN.get(
            self.robots[0].robot_model.__class__.__name__, {}
        ).get("peg", lambda x: x)
        ...
        body1 = find_elements(
            root=model0.worldbody,
            tags="body",
            attribs={"name": eef0},
            return_first=True,
        )
        body2 = find_elements(
            root=model1.worldbody,
            tags="body",
            attribs={"name": eef1},
            return_first=True,
        )
    ...
    def _setup_observables(self):
        ...
        if self.use_object_obs:
            ...
            if self.env_configuration == "single-robot":
                self.robots[0].robot_model.naming_prefix + "right_"
                self.robots[0].robot_model.naming_prefix + "left_"
            else:
                self.robots[0].robot_model.naming_prefix
                self.robots[1].robot_model.naming_prefix
            modality = "object"
            ...
            @sensor(modality=modality)
            def t(obs_cache):
                return obs_cache.get("t", 0.0)

            @sensor(modality=modality)
            def d(obs_cache):
                return obs_cache.get("d", 0.0)
