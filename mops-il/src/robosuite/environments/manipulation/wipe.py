...
class Wipe(ManipulationEnv):
    ...
    def reward(self, action=None):
        ...
        total_force_ee = max(
            [
                np.linalg.norm(
                    np.array(self.robots[0].recent_ee_forcetorques[arm].current[:3])
                )
        ...
        if (
            self.check_contact(self.robots[0].robot_model)
            or self.robots[0].check_q_limits()
        ):
            ...
            if self.reward_shaping:
                ...
                reward -= self.ee_accel_penalty * max(
                    [
                        np.mean(abs(self.robots[0].recent_ee_acc[arm].current))
                        for arm in self.robots[0].arms
                    ]
                )
        ...
        if self.print_results:
            string_to_print = (
                f"Process {id(multiprocessing.current_process())}, timestep {self.timestep:>4}: reward: {reward:8.4f}"
                f"wiped markers: {len(self.wiped_markers):>3} collisions: {self.collisions:>3} f-excess: {self.f_excess:>3}"
            )
    ...
    def _load_model(self) -> None:
        ...
        if self.delta_height is None:
            self.delta_height = self.rng.normal(
                self.table_height, self.table_height_std
            )
    ...
    def _setup_observables(self):
        ...
        if self.use_object_obs:
                ...
                if self.use_robot_obs:
                    ...
                    robot_obs_sensors = [
                        self._get_obj_eef_sensor(
                            full_pf,
                            "wipe_centroid",
                            f"{arm_pf}gripper_to_wipe_centroid",
                            modality,
                        )
                        for arm_pf, full_pf in zip(
                            arm_prefixes, full_prefixes, strict=False
                        )
                    ]
            ...
            # Create observables
            for name, s in zip(names, sensors, strict=False):
    ...
    def _create_marker_sensors(self, i, marker, modality="object"):
        ...
        if self.use_robot_obs:
            ...
            grippers_to_marker_fns = [
                self._get_obj_eef_sensor(
                    full_pf, f"marker{i}_pos", f"{arm_pf}gripper_to_marker{i}", modality
                )
                for arm_pf, full_pf in zip(arm_prefixes, full_prefixes, strict=False)
            ]
    ...
    def _post_action(self, action):
        ...
        # Update force bias
        if all(
            np.linalg.norm(self.ee_force_bias[arm]) == 0 for arm in self.ee_force_bias
        ):
    ...
    def _get_wipe_information(self):
        ...
        if len(self.wiped_markers) < self.num_markers:
            ...
            mean_pos_to_things_to_wipe_list = [
                wipe_centroid - self._get_eef_xpos(arm) for arm in self.robots[0].arms
            ]
    ...
    def _has_gripper_contact(self):
        ...
        """
        return any(
            np.linalg.norm(self.robots[0].ee_force[arm] - self.ee_force_bias[arm])
