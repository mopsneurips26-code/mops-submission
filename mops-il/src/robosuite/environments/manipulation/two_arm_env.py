...
class TwoArmEnv(ManipulationEnv):
    ...
    def _check_robot_configuration(self, robots) -> None:
        ...
        if self.env_configuration in self._PREV_CONFIG_TO_NEW_CONFIG:
            self.env_configuration = self._PREV_CONFIG_TO_NEW_CONFIG[
        ...
        if len(robots) == 1:
            if not check_bimanual(robots[0]):
                raise ValueError(
                    "Error: If inputting a single robot, it must be bimanual."
                )
    ...
    def _gripper0_to_target(self, target, target_type="body", return_distance=False):
        ...
        if self.env_configuration == "single-robot":
            return self._gripper_to_target(
                self.robots[0].gripper["right"], target, target_type, return_distance
            )
        else:
            return self._gripper_to_target(
                self.robots[0].gripper, target, target_type, return_distance
            )
    ...
    def _gripper1_to_target(self, target, target_type="body", return_distance=False):
        ...
        if self.env_configuration == "single-robot":
            return self._gripper_to_target(
                self.robots[0].gripper["left"], target, target_type, return_distance
            )
        else:
            return self._gripper_to_target(
                self.robots[1].gripper, target, target_type, return_distance
            )
    ...
    def _eef1_xpos(self):
        ...
        if self.env_configuration == "single-robot":
            ...
            return np.array(
                self.sim.data.site_xpos[self.robots[1].eef_site_id["right"]]
            )
