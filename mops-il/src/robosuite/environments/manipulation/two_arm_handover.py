            ...
            if self.env_configuration == "single-robot":
                ...
                arm_sensor_fns = [
                    self._get_obj_eef_sensor(
                        full_pf, "handle_xpos", f"gripper{i}_to_handle", modality
                    )
                    ...
                    for _j in range(100):
                        ...
                        if self.env_configuration == "single-robot":
                            ...
                            torques = np.concatenate(
                                [
                                    self.robots[0]
                                    .part_controllers["right"]
                                    .torque_compensation,
                                    self.robots[0]
                                    .part_controllers["left"]
                                    .torque_compensation,
                                ]
                            )
                            self.sim.data.ctrl[
                                self.robots[0]._ref_arm_joint_actuator_indexes
                            ] = torques
                        else:
                            robot_noops = [
                                np.concatenate(
                                    [
                                        robot.controller[arm].torque_compensation
                            ...
                            self.sim.data.ctrl[
                                self.robots[1]._ref_arm_joint_actuator_indexes
                            ] = robot_noops[1]
                        # Execute gripper action
                        gripper_ac = [1] * self.robots[0].gripper["right"].dof
                        gripper_ac = (
                            self.robots[0].gripper["right"].format_action(gripper_ac)
                        )
                        self.robots[0].part_controllers["right_gripper"].set_goal(
                            gripper_ac
                        )
                        # Take forward step
                        self.sim.step()
