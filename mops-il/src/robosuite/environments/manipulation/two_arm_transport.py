...
class TwoArmTransport(TwoArmEnv):
    ...
    def _load_model(self) -> None:
        ...
        if self.env_configuration == "single-robot":
            ...
            if self.env_configuration == "opposed":
                ...
                for robot, rotation, offset in zip(
                    self.robots, (np.pi / 2, -np.pi / 2), (-0.25, 0.25), strict=False
                ):
                ...
                for robot, offset in zip(self.robots, (-0.6, 0.6), strict=False):
                    xpos = robot.robot_model.base_xpos_offset["table"](
                        self.table_full_size[0]
                    )
        ...
        trash = BoxObject(
            name="trash", size=[0.02, 0.02, 0.02], material=redwood, rng=self.rng
        )
    ...
    def _get_placement_initializer(self) -> None:
            ...
            if self.env_configuration == "single-robot":
                ...
                arm_sensor_fns = [
                    self._get_obj_eef_sensor(
                        full_pf, "payload_pos", f"gripper{i}_to_payload", modality
                    )
                    for i, full_pf in enumerate(prefixes)
                ]
                arm_sensor_fns += [
                    self._get_obj_eef_sensor(
                        pf0, "lid_handle_pos", "gripper0_to_lid_handle", modality
                    ),
                    self._get_obj_eef_sensor(
                        pf1, "trash_pos", "gripper1_to_trash", modality
                    ),
                ]
            else:
                ...
                robot_arm_prefixes = [
                    self._get_arm_prefixes(robot, include_robot_name=False)
                    for robot in self.robots
                ]
                robot_full_prefixes = [
                    self._get_arm_prefixes(robot, include_robot_name=True)
                    for robot in self.robots
                ]
                for i, (arm_prefixes, full_prefixes) in enumerate(
                    zip(robot_arm_prefixes, robot_full_prefixes, strict=False)
                ):
                    arm_sensor_fns += [
                        self._get_obj_eef_sensor(
                            full_pf,
                            "payload_pos",
                            f"{arm_pf}gripper{i}_to_payload",
                            modality,
                        )
                    ...
                    if i == 0:
                        arm_sensor_fns += [
                            self._get_obj_eef_sensor(
                                full_pf,
                                "lid_handle_pos",
                                f"{arm_pf}gripper0_to_lid_handle",
                                modality,
                            )
                            for arm_pf, full_pf in zip(
                                arm_prefixes, full_prefixes, strict=False
    ...
    def _check_success(self) -> bool:
        ...
        """
        return bool(
            self.transport.payload_in_target_bin and self.transport.trash_in_trash_bin
        )
