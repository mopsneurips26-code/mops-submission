...
class ToolHang(ManipulationEnv):
    ...
    def _setup_references(self) -> None:
        ...
        for i in range(4):
            self.obj_geom_id[f"stand_wall_{i}"] = self.sim.model.geom_name2id(
                f"stand_wall{i}"
            )
        for i in range(self.tool_args["ngeoms"]):
            self.obj_geom_id[f"tool_hole1_hc_{i}"] = self.sim.model.geom_name2id(
                f"tool_hole1_hc_{i}"
            )
    ...
    def _setup_observables(self):
        ...
        if self.use_object_obs:
            ...
            sensors = [
                self._get_world_pose_in_gripper_sensor(
                    full_pf, f"world_pose_in_{arm_pf}gripper", modality
                )
    ...
    def _create_obj_sensors(
        ...
        sensors = [
            self._get_rel_obj_eef_sensor(
                arm_pf, obj_name, f"{obj_name}_to_{full_pf}eef_pos", full_pf, modality
            )
            for arm_pf, full_pf in zip(arm_prefixes, full_prefixes, strict=False)
        ]
        sensors += [
            self._get_obj_eef_rel_quat_sensor(
                full_pf, obj_name, f"{obj_name}_to_{full_pf}eef_quat", modality
            )
    ...
    def _check_frame_assembled(self):
        ...
        geom_positions = [
            self.sim.data.geom_xpos[self.obj_geom_id[f"stand_wall_{i}"]] - hook_endpoint
            for i in range(4)
        ]
    ...
    def _check_tool_on_frame(self):
        ...
        all_tool_hole_geoms = [
            f"tool_hole1_hc_{i}" for i in range(self.tool_args["ngeoms"])
        ]
        ...
        g2_pos = self.sim.data.geom_xpos[self.obj_geom_id[f"tool_hole1_hc_{g2_id}"]]
