...
class TwoArmLift(TwoArmEnv):
    ...
    def _load_model(self) -> None:
        ...
        if self.env_configuration == "single-robot":
            ...
            if self.env_configuration == "opposed":
                ...
                for robot, rotation in zip(
                    self.robots, (np.pi / 2, -np.pi / 2), strict=False
                ):
                ...
                for robot, offset in zip(self.robots, (-0.25, 0.25), strict=False):
                    xpos = robot.robot_model.base_xpos_offset["table"](
                        self.table_full_size[0]
                    )
            ...
            if self.env_configuration == "single-robot":
                ...
                arm_sensor_fns = [
                    self._get_obj_eef_sensor(
                        full_pf, f"handle{i}_xpos", f"gripper{i}_to_handle{i}", modality
                    )
                ...
                for i, (arm_prefixes, full_prefixes) in enumerate(
                    ...
                    arm_sensor_fns += [
                        self._get_obj_eef_sensor(
                            full_pf,
                            f"handle{i}_xpos",
                            f"{arm_pf}gripper{i}_to_handle{i}",
                            modality,
                        )
    ...
    def visualize(self, vis_settings) -> None:
        ...
        if vis_settings["grippers"]:
            ...
            for gripper, handle in zip(grippers, handles, strict=False):
    ...
    def _gripper0_to_handle0(self):
        ...
        """
        return self._gripper0_to_target(
            self.pot.important_sites["handle0"], target_type="site"
        )
    ...
    def _gripper1_to_handle1(self):
        ...
        """
        return self._gripper1_to_target(
            self.pot.important_sites["handle1"], target_type="site"
        )
