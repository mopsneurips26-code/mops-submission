...
def load_part_controller_config(custom_fpath=None, default_controller=None):
    ...
    if default_controller is not None:
        ...
        assert default_controller in ALL_PART_CONTROLLERS, (
            f"Error: Unknown default controller specified. Requested {default_controller}, "
            f"available controllers: {list(ALL_PART_CONTROLLERS)}"
        )
        ...
        custom_fpath = os.path.join(
            ...
            "..",
            f"config/default/parts/{default_controller.lower()}.json",
        )
    ...
    try:
        ...
        print(
            f"Error opening controller filepath at: {custom_fpath}. "
            "Please check filepath and try again."
        )
...
def controller_factory(part_name, controller_type, controller_params):
    if part_name in ["right", "left"]:
        ...
        raise ValueError(f"Unknown controller part name: {part_name}")
