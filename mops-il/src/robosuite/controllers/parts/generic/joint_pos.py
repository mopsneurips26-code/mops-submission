...
class JointPositionController(Controller):
    ...
    def __init__(
        ...
        **kwargs,  # does nothing; used so no error raised when dict is passed with extra terms used previously
        ...
        self.kd = (
            ...
            if kwargs.get("kd") is None
            else self.nums2array(kwargs.get("kd"), self.control_dim)
        )
    ...
    def set_goal(self, action, set_qpos=None) -> None:
        ...
        if self.input_type == "delta":
            ...
            scaled_delta = self.scale_action(delta) if delta is not None else None
