...
class JointVelocityController(Controller):
    ...
    def set_goal(self, velocities) -> None:
        ...
        assert len(velocities) == self.joint_dim, (
            f"Goal action must be equal to the robot's joint dimension space! Expected {self.joint_dim}, got {len(velocities)}"
        )
    ...
    def run_controller(self):
        ...
        self.saturated = np.sum(np.abs(self.torques - torques)) != 0
