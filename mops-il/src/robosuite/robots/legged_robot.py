...
class LeggedRobot(MobileRobot):
    ...
    def _load_leg_controllers(self) -> None:
        ...
        assert type(self.part_controller_config[self.legs]) == dict, (
            f"Inputted controller config must be a dict! Instead, got type: {type(self.part_controller_config[self.legs])}"
        )
        ...
        self.part_controller_config[self.legs]["naming_prefix"] = (
        ...
        ref_legs_joint_indexes = [
            self.sim.model.joint_name2id(x) for x in self.robot_model.legs_joints
        ]
        ref_legs_joint_pos_indexes = [
            self.sim.model.get_joint_qpos_addr(x) for x in self.robot_model.legs_joints
        ]
        ref_legs_joint_vel_indexes = [
            self.sim.model.get_joint_qvel_addr(x) for x in self.robot_model.legs_joints
        ]
        ...
        low = self.sim.model.actuator_ctrlrange[
            self._ref_actuators_indexes_dict[self.legs], 0
        ]
        high = self.sim.model.actuator_ctrlrange[
            self._ref_actuators_indexes_dict[self.legs], 1
        ]
    ...
    def _load_controller(self) -> None:
        ...
        self.composite_controller = composite_controller_factory(
            ...
            grippers={
                self.get_gripper_name(arm): self.gripper[arm] for arm in self.arms
            },
        )
        ...
        self.composite_controller.load_controller_config(
            ...
            self.composite_controller_config.get(
    ...
    def reset(self, deterministic=False, rng=None) -> None:
        ...
        if isinstance(self.robot_model.base, LegBaseModel):
            ...
            self.sim.data.qpos[self._ref_legs_joint_pos_indexes] = (
                self.robot_model.base.init_qpos
            )
    ...
    def setup_references(self) -> None:
        ...
        self._ref_actuators_indexes_dict[self.legs] = [
            ...
            for actuator in self.robot_model.legs_actuators
        ]

        self._ref_joints_indexes_dict[self.legs] = [
            self.sim.model.joint_name2id(joint)
            for joint in self.robot_model.legs_joints
        ]

        self._ref_legs_joint_pos_indexes = [
            self.sim.model.get_joint_qpos_addr(x) for x in self.robot_model.legs_joints
        ]
        self._ref_legs_joint_vel_indexes = [
            self.sim.model.get_joint_qvel_addr(x) for x in self.robot_model.legs_joints
        ]

    def control(self, action, policy_step=False) -> None:
        ...
        assert len(action) == self.action_dim, (
            f"environment got invalid action dimension -- expected {self.action_dim}, got {len(action)}"
        )
        ...
        for part_name, applied_action in applied_action_dict.items():
            applied_action_low = self.sim.model.actuator_ctrlrange[
                self._ref_actuators_indexes_dict[part_name], 0
            ]
            applied_action_high = self.sim.model.actuator_ctrlrange[
                self._ref_actuators_indexes_dict[part_name], 1
            ]
            ...
            applied_action = np.clip(
                applied_action / actuator_gears, applied_action_low, applied_action_high
            )
        ...
        if policy_step:
            ...
            for arm in self.arms:
                ...
                self.recent_ee_pose[arm].push(
                    np.concatenate(
                        (controller.ref_pos, T.mat2quat(controller.ref_ori_mat))
                    )
                )
                self.recent_ee_vel[arm].push(
                    np.concatenate((controller.ref_pos_vel, controller.ref_ori_vel))
                )
