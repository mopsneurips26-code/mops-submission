...
class Robot:
    ...
    def __init__(
        ...
        if composite_controller_config is not None:
            ...
            self.composite_controller_config = load_composite_controller_config(
                robot=robot_type
            )

        self.part_controller_config = copy.deepcopy(
            self.composite_controller_config.get("body_parts", {})
        )
    ...
    def _load_controller(self) -> Never:
        """Loads controller to be used for dynamic trajectories."""
        raise NotImplementedError

    def _postprocess_part_controller_config(self) -> None:
    ...
    def load_model(self) -> None:
        ...
        if self.base_type == "default":
            self.robot_model.add_base(
                base=robot_base_factory(self.robot_model.default_base, idn=self.idn)
            )
        else:
            self.robot_model.add_base(
                base=robot_base_factory(self.base_type, idn=self.idn)
            )
        ...
        for arm in self.arms:
            if self.has_gripper[arm]:
                if self.gripper_type[arm] == "default":
                    ...
                    self.gripper[arm] = gripper_factory(
                        self.robot_model.default_gripper[arm], idn=idn
                    )
                else:
            ...
            custom_gripper_mount_pos_offset = (
                self.robot_model.gripper_mount_pos_offset.get(arm, None)
            )
            custom_gripper_mount_quat_offset = (
                self.robot_model.gripper_mount_quat_offset.get(arm, None)
            )
            ...
            if custom_gripper_mount_pos_offset is not None:
                ...
                self.gripper[arm].worldbody.find("body").attrib["pos"] = (
                    array_to_string(custom_gripper_mount_pos_offset)
                )
            if custom_gripper_mount_quat_offset is not None:
                assert len(custom_gripper_mount_quat_offset) == 4
                self.gripper[arm].worldbody.find("body").attrib["quat"] = (
                    array_to_string(custom_gripper_mount_quat_offset)
                )
    ...
    def reset_sim(self, sim: MjSim) -> None:
    ...
    def reset(self, deterministic=False, rng=None) -> None:
        ...
        if not deterministic:
            ...
            if self.initialization_noise["type"] == "gaussian":
                noise = (
                    rng.standard_normal(len(self.init_qpos))
                    * self.initialization_noise["magnitude"]
                )
            elif self.initialization_noise["type"] == "uniform":
                noise = (
                    rng.uniform(-1.0, 1.0, len(self.init_qpos))
                    * self.initialization_noise["magnitude"]
                )
            else:
                raise ValueError(
        ...
        if self.robot_model.init_base_qpos is not None:
            self.sim.data.qpos[self._ref_base_joint_pos_indexes] = (
                self.robot_model.init_base_qpos
            )
        ...
        self.base_ori = self.sim.data.get_body_xmat(self.robot_model.root_body).reshape(
            (3, 3)
        )
    ...
    def setup_references(self) -> None:
        ...
        self._ref_joint_indexes = [
            self.sim.model.joint_name2id(joint) for joint in self.robot_joints
        ]

        # indices for joint pos actuation, joint vel actuation, gripper actuation
        self._ref_arm_joint_actuator_indexes = [
            self.sim.model.actuator_name2id(actuator)
            for actuator in self.robot_model.arm_actuators
        ]

        self.robot_arm_joints = self.robot_model.arm_joints
        self._ref_arm_joint_indexes = [
            self.sim.model.joint_name2id(joint) for joint in self.robot_arm_joints
        ]
        ...
        self._ref_torso_joint_pos_indexes = [
            ...
            for x in self.robot_model._torso_joints
        ]
    ...
    def setup_observables(self):
        ...
        def joint_acc(obs_cache):
            return np.array(
                [self.sim.data.qacc[x] for x in self._ref_joint_vel_indexes]
            )
        ...
        for name, s, active in zip(names, sensors, actives, strict=False):
            obs_name = pf + name
    ...
    def _create_arm_sensors(self, arm, modality):
        ...
        def eef_quat_site(obs_cache):
            ...
            """
            return T.mat2quat(
                self.sim.data.site_xmat[self.eef_site_id[arm]].reshape((3, 3))
            )
    ...
    def control(self, action, policy_step=False) -> Never:
    ...
    def check_q_limits(self) -> bool:
        ...
        for qidx, (q, q_limits) in enumerate(
            ...
            if q_limits[0] != q_limits[1] and not (
                ...
                ROBOSUITE_DEFAULT_LOGGER.warning(
                    "Joint limit reached in joint " + str(qidx)
                )
                return True
        return False
    ...
    def _input2dict(self, inp):
        ...
        return {
            ...
            for key, value in zip(self.arms, inp, strict=False)
        }
    ...
    def torque_limits(self):
        ...
        high = self.sim.model.actuator_ctrlrange[
            self._ref_arm_joint_actuator_indexes, 1
        ]
        return low, high
    ...
    def set_robot_joint_positions(self, jpos) -> None:
    ...
    def set_gripper_joint_positions(
        self, jpos: np.ndarray, gripper_arm: str | None = None
    ) -> None:
    ...
    def _visualize_grippers(self, visible) -> None:
    ...
    def _load_arm_controllers(self) -> None:
        urdf_loaded = False
        ...
        for arm in self.arms:
            ...
            assert type(self.part_controller_config[arm]) == dict, (
                f"Inputted controller config must be a dict! Instead, got type: {type(self.part_controller_config[arm])}"
            )
            ...
            self.part_controller_config[arm]["ref_name"] = self.gripper[
                arm
            ].important_sites["grip_site"]
            self.part_controller_config[arm]["part_name"] = arm
            self.part_controller_config[arm]["naming_prefix"] = (
                self.robot_model.naming_prefix
            )

            self.part_controller_config[arm]["eef_rot_offset"] = self.eef_rot_offset[
                arm
            ]
            ...
            self.part_controller_config[arm]["load_urdf"] = bool(not urdf_loaded)
            urdf_loaded = True
            ...
            if self.has_gripper[arm]:
                ...
                assert "gripper" in self.part_controller_config[arm], (
                    "Gripper controller config not found!"
                )
                gripper_name = self.get_gripper_name(arm)
                self.part_controller_config[gripper_name] = {}
                self.part_controller_config[gripper_name]["type"] = (
                    self.part_controller_config[arm]["gripper"]["type"]
                )
                self.part_controller_config[gripper_name]["use_action_scaling"] = (
                    self.part_controller_config[arm]["gripper"].get(
                ...
                self.part_controller_config[gripper_name]["eef_name"] = self.gripper[
                    arm
                ].important_sites["grip_site"]
                ...
                self.part_controller_config[gripper_name]["ndim"] = self.gripper[
                    arm
                ].dof
                ...
                low = self.sim.model.actuator_ctrlrange[
                    self._ref_joint_gripper_actuator_indexes[arm], 0
                ]
                high = self.sim.model.actuator_ctrlrange[
                    self._ref_joint_gripper_actuator_indexes[arm], 1
                ]
    ...
    def enable_parts(self, right=True, left=True) -> None:
        self._enabled_parts = {
            "right": right,
            "right_gripper": right,
            "left": left,
    ...
    def print_action_info_dict(self) -> None:
        if self.composite_controller is not None and hasattr(
            self.composite_controller, "print_action_info_dict"
        ):
    ...
    def get_gripper_name(self, arm) -> str:
        return f"{arm}_gripper"

    def has_part(self, part_name) -> bool:
        return bool(
            part_name in self._ref_joints_indexes_dict
            and len(self._ref_joints_indexes_dict[part_name]) > 0
        )
