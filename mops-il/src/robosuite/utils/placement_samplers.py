...
class ObjectPositionSampler:
    ...
    def add_objects(self, mujoco_objects) -> None:
    ...
    def sample(self, fixtures=None, reference=None, on_top=True) -> Never:
...
class UniformRandomSampler(ObjectPositionSampler):
    ...
    def sample(self, fixtures=None, reference=None, on_top=True):
        ...
        for obj in self.mujoco_objects:
            ...
            success = False
            for _i in range(5000):  # 5000 retries
...
class SequentialCompositeSampler(ObjectPositionSampler):
    ...
    def append_sampler(self, sampler, sample_args=None) -> None:
    ...
    def hide(self, mujoco_objects) -> None:
    ...
    def add_objects(self, mujoco_objects) -> Never:
    ...
    def add_objects_to_sampler(self, sampler_name, mujoco_objects) -> None:
        ...
        assert sampler_name in self.samplers, (
            f"Invalid sub-sampler specified, valid options are: {self.samplers.keys()}, "
            f"requested: {sampler_name}"
        )
    ...
    def sample(self, fixtures=None, reference=None, on_top=True):
        ...
        for sampler, s_args in zip(
            self.samplers.values(), self.sample_args.values(), strict=False
        ):
            # Pre-process sampler args
            if s_args is None:
                s_args = {}
            for arg_name, arg in zip(
                ("reference", "on_top"), (reference, on_top), strict=False
            ):
