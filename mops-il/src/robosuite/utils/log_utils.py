...
MESSAGE_STR = "%(message)s (%(pathname)s:%(lineno)d)"
...
class ConsoleFormatter(logging.Formatter):
    ...
    FORMATS = {
        ...
        logging.INFO: colored(FORMAT_STR["console"], "green", attrs=["bold"])
        + MESSAGE_STR,
...
class DefaultLogger:
    ...
    def __init__(
        ...
        if file_logging_level is not None:
            ...
            log_file_path = f"/tmp/robosuite_{time_str}_{os.getpid()}.log"
            fh = logging.FileHandler(log_file_path)
            print(colored(f"[robosuite]: Saving logs to {log_file_path}", "yellow"))
...
def format_message(level: str, message: str) -> str:
    ...
    formatted_message = colored(
        f"[{level}] {filename}:{lineno} - {message}", color, attrs=attrs
    )
    return formatted_message


def rs_assert(condition: bool, message: str) -> None:
