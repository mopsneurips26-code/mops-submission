...
class Interpolator(metaclass=abc.ABCMeta):
    ...
    @abc.abstractmethod
    def get_interpolated_goal(self) -> Never:
...
class LinearInterpolator(Interpolator):
    ...
    def set_states(self, dim=None, ori=None) -> None:
    ...
    def set_goal(self, goal) -> None:
        ...
        if goal.shape[0] != self.dim:
            ...
            raise ValueError(
                f"LinearInterpolator: Input size wrong for goal; got {goal.shape[0]}, needs to be {self.dim}!"
            )
