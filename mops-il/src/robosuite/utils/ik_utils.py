...
class IKSolver:
    ...
    def get_controller_base_pose(self):
        ...
        return T.make_pose(
            ...
            rotation=T.quat2mat(
                np.roll(self.full_model_data.body(self.input_ref_frame).xquat, shift=-1)
            ),
        )
    ...
    def set_target_positions(self, target_positions: list) -> None:
        for i, pos in enumerate(target_positions):
            self.full_model_data.mocap_pos[self.mocap_ids[i]] = pos

    def set_target_rotations(self, target_rotations: list) -> None:
    ...
    def forward_kinematics(self, qpos: np.ndarray) -> dict[str, np.ndarray]:
        ...
        return {
            ...
            for name, site_id in zip(self.site_names, self.site_ids, strict=False)
        }
        ...
        if dst_frame == "world":
            ...
            X_dst_frame_W = np.linalg.inv(
                T.make_pose(
                    ...
                    rotation=T.quat2mat(
                        np.roll(self.full_model.body(dst_frame).quat, shift=-1)
                    ),
                )
    ...
    def solve(
        ...
        if self.input_ref_frame != "world":
            ...
            for i in range(len(target_pos)):
                ...
                X_W_goal = self.transform_pose(
                    X_B_goal, src_frame=src_frame, dst_frame="world"
                )
