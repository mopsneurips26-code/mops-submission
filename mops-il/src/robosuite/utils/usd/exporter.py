...
try:
    ...
    raise AssertionError(
        "Please install usd-core package. You can install it using 'pip install usd-core'."
    )
import scipy
import termcolor
import tqdm
...
"""
    ...
    def _initialize_output_directories(self) -> None:
        ...
        if self.verbose:
            print(
                termcolor.colored(
                    f"Writing output frames and assets to {self.output_directory_path}",
                    "green",
                )
            )
    ...
    def _update_scene(
        ...
        if not isinstance(camera, _structs.MjvCamera):
            ...
            if isinstance(camera_id, str):
                ...
                if camera_id == -1:
                    raise ValueError(
                        f'The camera "{camera}" does not exist. Valid cameras {self.valid_camera_names}'
                    )
            if camera_id == -1:
    ...
    def _load_geom(self, geom: mujoco.MjvGeom) -> None:
    ...
    def add_light(
        ...
        light_type: str | None = "sphere",
        light_name: str | None = "light_1",
        **light_params,
    ) -> None:
        ...
        if light_type == "sphere":
            assert "radius" in light_params, (
                "Please provide a radius for the sphere light."
            )
            ...
            assert "height" in light_params, (
                "Please provide a height for the rect light."
            )
            width = light_params["width"]
            height = light_params["height"]

            new_light = light_module.USDRectLight(
                stage=self.stage, light_name=light_name, width=width, height=height
            )

            new_light.update(pos=pos, intensity=intensity, color=color, frame=0)
        elif light_type == "cylinder":
            assert "length" in light_params, (
                "Please provide a length for the cylinder light."
            )
            assert "radius" in light_params, (
                "Please provide a radius for the cylinder light."
            )
