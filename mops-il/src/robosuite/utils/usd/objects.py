...
class USDObject(abc.ABC):
    ...
    * `_get_mesh_geometry(self)`: gets the mesh geometry of an object
    ...
    @abc.abstractmethod
    def _get_uv_geometry(self) -> Never:
        """Gets UV information for an object in the scene."""
        raise NotImplementedError

    @abc.abstractmethod
    def _get_mesh_geometry(self) -> Never:
    ...
    def _set_refinement_properties(self, usd_prim, scheme="none") -> None:
    ...
    def update_visibility(self, visible: bool, frame: int | None = None) -> None:
        """Updates the visibility of an object in a scene for a given frame."""
        visibility_setting = "inherited" if visible else "invisible"
        utils_module.set_attr(
            attr=self.usd_xform.GetVisibilityAttr(),
            value=visibility_setting,
            frame=frame,
        )

    def update_scale(self, scale: np.ndarray, frame: int | None = None) -> None:
        """Updates the scale of an object in the scene for a given frame."""
        utils_module.set_attr(
            attr=self.scale_op, value=Gf.Vec3f(scale.tolist()), frame=frame
        )
...
class USDTendon(USDObject):
    ...
    def update_scale(self, scale: np.ndarray, frame: int | None) -> None:
        """Updates the scale of the tendon."""
        for name in self.tendon_parts:
            if "cylinder" in name:
                utils_module.set_attr(
                    attr=self.usd_refs[name]["scale_op"],
