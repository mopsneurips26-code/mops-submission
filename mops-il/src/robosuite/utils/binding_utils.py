...
class MjRenderContext:
    ...
    def __init__(
        ...
        if _MUJOCO_GL not in ("disable", "disabled", "off", "false", "0"):
            ...
            if _SYSTEM == "Linux" and _MUJOCO_GL == "osmesa":
                ...
                from robosuite.renderers.context.glfw_context import (
    ...
    def update_offscreen_size(self, width, height) -> None:
    ...
    def render(self, width, height, camera_id=None, segmentation=False) -> None:
    ...
    def upload_texture(self, tex_id) -> None:
...
class MjRenderContextOffscreen(MjRenderContext):
    def __init__(self, sim, device_id, max_width=640, max_height=480) -> None:
        super().__init__(
            sim,
            offscreen=True,
            device_id=device_id,
...
class MjSimState:
    ...
    def __init__(self, time, qpos, qvel) -> None:
        self.time = time
        self.qpos = qpos
        self.qvel = qvel
...
class MjModel(metaclass=_MjModelMeta):
    ...
    def _extract_mj_names(self, name_adr, num_obj, obj_type):
        ...
        id2name = dict.fromkeys(range(num_obj))
        name2id = {}
    ...
    def body_name2id(self, name):
        ...
        if name not in self._body_name2id:
            raise ValueError(
                f'No "body" with name {name} exists. Available "body" names = {self.body_names}.'
            )
    ...
    def joint_name2id(self, name):
        ...
        if name not in self._joint_name2id:
            raise ValueError(
                f'No "joint" with name {name} exists. Available "joint" names = {self.joint_names}.'
            )
    ...
    def geom_name2id(self, name):
        ...
        if name not in self._geom_name2id:
            raise ValueError(
                f'No "geom" with name {name} exists. Available "geom" names = {self.geom_names}.'
            )
    ...
    def site_name2id(self, name):
        ...
        if name not in self._site_name2id:
            raise ValueError(
                f'No "site" with name {name} exists. Available "site" names = {self.site_names}.'
            )
    ...
    def light_name2id(self, name):
        ...
        if name not in self._light_name2id:
            raise ValueError(
                f'No "light" with name {name} exists. Available "light" names = {self.light_names}.'
            )
    ...
    def actuator_name2id(self, name):
        ...
        if name not in self._actuator_name2id:
            raise ValueError(
                f'No "actuator" with name {name} exists. Available "actuator" names = {self.actuator_names}.'
            )
    ...
    def sensor_name2id(self, name):
        ...
        if name not in self._sensor_name2id:
            raise ValueError(
                f'No "sensor" with name {name} exists. Available "sensor" names = {self.sensor_names}.'
            )
    ...
    def tendon_name2id(self, name):
        ...
        if name not in self._tendon_name2id:
            raise ValueError(
                f'No "tendon" with name {name} exists. Available "tendon" names = {self.tendon_names}.'
            )
    ...
    def mesh_name2id(self, name):
        ...
        if name not in self._mesh_name2id:
            raise ValueError(
                f'No "mesh" with name {name} exists. Available "mesh" names = {self.mesh_names}.'
            )
    ...
    def get_xml(self):
        with TemporaryDirectory() as td:
            ...
            mujoco.mj_saveLastXML(filename.encode(), self._model)
...
class MjData(metaclass=_MjDataMeta):
    ...
    def set_mocap_pos(self, name, value) -> None:
    ...
    def set_mocap_quat(self, name, value) -> None:
        """Set the quaternion of a mocap body using name.
    ...
    def set_joint_qpos(self, name, value) -> None:
    ...
    def set_joint_qvel(self, name, value) -> None:
...
class MjSim:
    ...
    def step(self, with_udd=True) -> None:
        """Step simulation."""
    ...
    def add_render_context(self, render_context) -> None:
    ...
    def set_state_from_flattened(self, value) -> None:
