...
def set_goal_position(delta, current_position, position_limit=None, set_pos=None):
    ...
    goal_position = set_pos if set_pos is not None else current_position + delta

    if position_limit is not None:
        if position_limit.shape != (2, n):
            raise ValueError(
                f"Position limit should be shaped (2,{n}) but is instead: {position_limit.shape}"
            )
...
def set_goal_orientation(
    ...
    if np.array(orientation_limit).any():
        if orientation_limit.shape != (2, 3):
            raise ValueError(
                f"Orientation limit should be shaped (2,3) but is instead: {orientation_limit.shape}"
            )
