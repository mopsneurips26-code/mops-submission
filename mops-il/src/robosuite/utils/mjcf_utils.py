    ...
    def __init__(
        ...
        if type(texture) is str:
            ...
            if not texture_is_path:
                assert texture in ALL_TEXTURES, (
                    f"Error: Requested invalid texture. Got {texture}. Valid options are:\n{ALL_TEXTURES}"
                )
        else:
            default = True
            ...
            if texture is not None:
                assert len(texture) == 4, (
                    f"Error: Requested default texture. Got array of length {len(texture)}."
                    "Expected rgba array of length 4."
                )
...
def convert_to_string(inp):
    ...
    if type(inp) in {list, tuple, np.ndarray}:
        ...
        raise ValueError(f"Unsupported type received: got {type(inp)}")


def set_alpha(node, alpha=0.1) -> None:
...
def get_size(size, size_max, size_min, default_max, default_min, rng=None):
    ...
    if len(default_max) != len(default_min):
        raise ValueError(
            f"default_max = {str(default_max)} and default_min = {str(default_min)}"
            + " have different lengths"
        )
    if size is not None:
        if (size_max is not None) or (size_min is not None):
            raise ValueError(
                f"size = {size} overrides size_max = {size_max}, size_min = {size_min}"
            )
    else:
        if size_max is None:
        ...
        size = np.array(
            [rng.uniform(size_min[i], size_max[i]) for i in range(len(default_max))]
        )
...
def recolor_collision_geoms(root, rgba, exclude=None) -> None:
...
def _element_filter(element, parent) -> str | None:
...
def find_elements_by_substring(
    root, tags, substrings, attribs=None, return_first=False
):
...
def save_sim_model(sim, fname) -> None:
...
def get_ids(sim, elements, element_type="geom", inplace=False):
    ...
    for i, element in enumerate(elements):
        if type(element) is not int:
            element_func = sim.model.__getattribute__(f"{element_type}_name2id")
...
def scale_geom_element(element, scale_array) -> None:
...
def scale_mesh_element(element, scale_array) -> None:
    ...
    m_scale = string_to_array(m_scale) if m_scale is not None else np.ones(3)
    m_scale *= scale_array
    element.set("scale", array_to_string(m_scale))


def scale_body_element(element, scale_array) -> None:
...
def scale_joint_element(element, scale_array, scale_slide_joints=True) -> None:
    ...
    if scale_slide_joints:
        j_type = element.get(
            "type", "hinge"
        )  # Default joint type is 'hinge' if not specified
...
def scale_site_element(element, scale_array) -> None:
...
def scale_mjcf_model(
    obj, asset_root, scale, get_elements_func, worldbody=None, scale_slide_joints=True
):
