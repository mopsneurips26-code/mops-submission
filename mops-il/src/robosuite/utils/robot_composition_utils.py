...
def create_composite_robot(
    ...
    robot: str,
    base: str | None = None,
    grippers: str | list[str] | tuple[str] | None = None,
) -> RobotModel:
    """Factory function to create a composite robot."""
    bimanual_robot = check_bimanual(robot)
    grippers = (
        grippers if type(grippers) == list or type(grippers) == tuple else [grippers]
    )
    ...
    if robot in ["Tiago", "GR1"] and base:
        ROBOSUITE_DEFAULT_LOGGER.warning(
            f"Defined custom base when using {robot} robot. Ignoring base."
        )
        if robot in ["Tiago"]:
            base = "NullMobileBase"
        elif robot == "GR1":
    ...
    class_dict = {
        ...
        "default_gripper": property(
            lambda self: {"right": grippers[0], "left": grippers[1]}
            if bimanual_robot
            else {"right": grippers[0]}
        ),
    }
