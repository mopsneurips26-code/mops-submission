...
class KeyboardHandler:
    def __init__(self, env, delta=0.05) -> None:
    ...
    def on_press(self, key) -> None:
        ...
        try:
            if key == Key.up:
                ...
                print(f"Delta now = {self.delta:.3f}")
            elif key == Key.left:
            ...
            # controls for setting active arm
                ...
                print(
                    f"Joint Indexes are 1-Indexed. Available joints are 1 - {self.num_joints}"
                )
            elif key.char == "1":
                ...
                if self._check_valid_joint(1):
                    self.active_arm_joint = 1
                    # Print out to user
                    print(f"New joint being tuned: {self.active_arm_joint}")
            elif key.char == "2":
                # Make sure range is valid; if so, update this specific joint
    ...
    def set_joint_positions(self, qpos) -> None:
    ...
    def _check_valid_joint(self, i) -> bool:
        ...
        if i > self.num_joints:
            # Print error
            print(
                f"Error: Requested joint {i} is out of range; available joints are 1 - {self.num_joints}"
            )
            return False
        else:
            return True
    ...
    def _toggle_arm(self) -> None:
        ...
        print(f"New robot arm being tuned: {robot}")

    def _update_joint_position(self, i, delta) -> None:
        ...
        print(f"Robot {robot} joint qpos: {self.current_joints_pos}")
