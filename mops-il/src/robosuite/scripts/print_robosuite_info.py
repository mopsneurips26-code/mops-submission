...
def bold_green_text(text) -> str:
    return f"\033[1m\033[32m{text}\033[0m"


def get_base_type(robot):
    return (
        ROBOT_CLASS_MAPPING[robot].__name__ if robot in ROBOT_CLASS_MAPPING else "N/A"
    )
...
info_sorted = dict(
    sorted(info.items(), key=lambda item: item[1].get("Base_Type", "zzz"))
)
