...
class RobosuiteEnvInterface:
    ...
    def load_robosuite_environment(self):
        ...
        env_meta = get_env_metadata_from_dataset(
            dataset_path=self.dataset, ds_format=args.ds_format
        )
        ...
        if env_meta["env_version"] < "v1.5.0":
            ...
            robot = (
                env_kwargs["robots"][0]
                if isinstance(env_kwargs["robots"], list)
                else env_kwargs["robots"]
            )
    ...
    def get_states(self):
        ...
        states = f[f"data/{ep}/states"][()][:: args.skip_frames]
        ...
        if self.reload_model:
            ...
            for name in self.keep_models:
                ...
                body_model = root_model.find(f".//body[@name='{name}']")
                body_env = root_env.find(f".//body[@name='{name}']")
    ...
    def link_env_with_ov(self):
        ...
        exp.add_light(
            pos=[0, 0, 0], intensity=1500, light_type="dome", light_name="dome_1"
        )
    ...
    def update_simulation(self, index) -> None:
    ...
    def write(self, data: dict) -> None:
        ...
        if self.write_ready:
            for annotator_name, annotator_data in data["annotators"].items():
                for idx, (_render_product_name, anno_rp_data) in enumerate(
                    annotator_data.items()
                ):
                    if annotator_name == "rgb":
                        ...
                        filepath = os.path.join(
                            args.cameras[idx],
                            "normals",
                            f"normals_{self._frame_id}.{self._image_output_format}",
                        )
...
class DataGenerator:
    ...
    def _check_if_valid_resolution(self, resolution) -> bool:
    ...
    def init_recorder(self) -> bool:
        ...
        for camera_name in args.cameras:
            ...
            if self._check_if_valid_camera(
                camera_path
            ) and self._check_if_valid_resolution(resolution):
    ...
    def finish_recording(self) -> None:
        ...
        print(
            f"[SDR] Finished;\tWrote {self.current_frame} frames to: {self.output_dir};"
        )
        self.clear_recorder()
    ...
    def create_video_from_frames(self, frame_folder, output_path, fps=30) -> None:
        frames = [f for f in os.listdir(frame_folder) if f.endswith(".png")]
        frames.sort(key=self.natural_sort_key)
        assert len(frames) == self.current_frame, (
    ...
    def create_video(self, videos_folder, camera, data_type) -> None:
        camera_folder_path = os.path.join(
            self.output_dir, camera, data_type
        )  # temp, change to render type
...
def main() -> None:
    ...
    if not total_episodes:
        ...
        total_episodes = [int(k.split("_")[1]) for k in f["data"]]
    f.close()
    ...
    for episode in total_episodes:
        output_directory = os.path.join(
            args.output_directory, dataset_name, "demo_" + str(episode)
        )
