...
def check_xml_definition(root):
    logger.info("Successfully loaded the xml file of robot model.")
    ...
    for part_name in ["torso", "head", "leg", "gripper", "base"]:
        ...
        for joint in world_body.findall(".//joint"):
            if (
                part_name in joint.attrib["name"]
                and joint.attrib["name"] not in added_joints
            ):
    ...
    total_counted_joints = sum([len(parts_dict[part_name]) for part_name in parts_dict])
    if total_counted_joints != len(world_body.findall(".//joint")):
        logger.error(
            f"Counted {total_counted_joints} joints, but found {len(world_body.findall('.//joint'))} joints"
        )
    else:
        logger.info("Your joint definition aligns with robosuite convention.")
        # remove empty list entries
        parts_dict = {
            part_name: joints for part_name, joints in parts_dict.items() if joints
        }
        print(
            f"    • Robosuite will be able to detect the following body parts:  {list(parts_dict.keys())}"
        )
        print("    • For each body part, you have defined the following joints:")
        for part_name in parts_dict:
            print(
                f"        • {part_name} - {len(parts_dict[part_name])} joints: {parts_dict[part_name]}"
            )
    return parts_dict
...
def check_registered_robot(robot_name) -> None:
    ...
    robot = Robot(
        ...
        composite_controller_config=controller_config,
        gripper_type=None,
    )
    logger.info("Succcessfully found the defined robot")
    ...
    if parts_dict.get("arm") is not None and parts_dict["arm"] != []:
        ...
        mount_names = [
            robot.robot_model._eef_name[name] for name in robot.robot_model._eef_name
        ]
        world_body = root.find(".//worldbody")
        for mount_name in mount_names[:num_arms]:
            ...
            if mount_body is None:
                ...
                exit(1)
        print(
            "        • Grippers will be mounted on the following bodies ",
            mount_names[:num_arms],
        )
