...
class MJGUI(Device):
    ...
    def __init__(self, env, active_end_effector: str | None = "right") -> None:
        super().__init__(env)
    ...
    def _get_site_names(self) -> list[str]:
        ...
        """
        if isinstance(
            self.env.robots[0].composite_controller, WholeBody
        ):  # input type passed to joint_action_policy
            site_names = self.env.robots[
                0
            ].composite_controller.joint_action_policy.site_names
        else:
    ...
    def _reset_internal_state(self) -> None:
        ...
        for site_name in site_names:
            target_name_prefix = (
                "right" if "right" in site_name else "left"
            )  # hardcoded for now
            target_pos = self.env.sim.data.site_xpos[
                self.env.sim.model.site_name2id(site_name)
            ]
            target_mat = self.env.sim.data.site_xmat[
                self.env.sim.model.site_name2id(site_name)
            ]
            set_mocap_pose(
                self.env.sim, target_pos, target_mat, f"{target_name_prefix}_eef_target"
            )
    ...
    def input2action(self, goal_update_mode="desired") -> dict[str, np.ndarray]:
        ...
        """
        assert goal_update_mode == "desired", (
        ...
        for site_name in site_names:
            ...
            target_pos_world, target_ori_mat_world = get_mocap_pose(
                self.env.sim, f"{target_name_prefix}_eef_target"
            )
            ...
            if isinstance(self.env.robots[0].composite_controller, WholeBody):
                ...
                assert (
                    self.env.robots[0]
                    .part_controllers[self.active_end_effector]
                    .input_ref_frame
                    == "world"
                    and self.env.robots[0]
                    .part_controllers[self.active_end_effector]
                    .input_type
                    == "absolute"
                ), (
            ...
            axis_angle_target = transform_utils.quat2axisangle(
                transform_utils.mat2quat(target_ori_mat)
            )
            action[target_name_prefix + "_abs"] = np.concatenate(
                [target_pos, axis_angle_target]
            )
