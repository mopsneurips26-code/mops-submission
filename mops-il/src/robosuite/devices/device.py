import abc
from typing import Never  # for abstract base class definitions
...
class Device(metaclass=abc.ABCMeta):
    ...
    def _reset_internal_state(self) -> None:
        ...
        self.grasp_states = [
            [False] * len(self.all_robot_arms[i]) for i in range(self.num_robots)
        ]
        self.active_arm_indices = [0] * len(self.all_robot_arms)
        self.active_robot = 0
        self.base_modes = [False] * len(self.all_robot_arms)

        self._prev_target = dict.fromkeys(self.all_robot_arms[self.active_robot])
    ...
    def active_arm_index(self, value) -> None:
        self.active_arm_indices[self.active_robot] = value

    @abc.abstractmethod
    def start_control(self) -> Never:
    ...
    def _postprocess_device_outputs(self, dpos, drotation) -> Never:
        raise NotImplementedError

    def input2action(
        self, mirror_actions=False, goal_update_mode="target"
    ) -> dict | None:
        ...
        dpos, _rotation, raw_drotation, grasp, reset = (
            state["dpos"],
            state["rotation"],
            state["raw_drotation"],
            state["grasp"],
        ...
        assert controller.name in ["OSC_POSE", "JOINT_POSITION"], (
            "only supporting OSC_POSE and JOINT_POSITION for now"
        )
        ...
        if robot.is_mobile:
            ...
            if base_mode is True:
                ...
                device_torso_input = dpos[
                    2
                ]  # Use vertical movement for torso in base mode
            else:
        ...
        if hasattr(robot, "torso") and robot.torso is not None:
            ...
            if (
                self.env.robots[0]
                .composite_controller.part_controllers["torso"]
                .joint_dim
                == 1
            ):
        ...
        if hasattr(gripper, "grasp_qpos"):
            ac_dict[f"{active_arm}_gripper"] = gripper.grasp_qpos[grasp]
        else:
    ...
    def get_arm_action(self, robot, arm, norm_delta, goal_update_mode="target"):
        ...
        assert goal_update_mode in [
            ...
            f"goal_update_mode must be either 'achieved' or 'target', got {goal_update_mode}"
        ...
        if isinstance(robot.part_controllers[arm], OperationalSpaceController):
            ...
            abs_action = arm_controller.delta_to_abs_action(
                delta_action, goal_update_mode=None
            )
            return {
                "delta": norm_delta,
                "abs": abs_action,
            }
        elif robot.composite_controller_config["type"] in [
            "WHOLE_BODY_MINK_IK",
            "HYBRID_WHOLE_BODY_MINK_IK",
        ]:
            ref_frame = self.env.robots[
                0
            ].composite_controller.composite_controller_specific_config.get(
            ...
            if goal_update_mode == "achieved" or self._prev_target[arm] is None:
                ...
                if ref_frame == "base":
                    ...
                    pose_in_base = self.env.robots[
                        0
                    ].composite_controller.joint_action_policy.transform_pose(
            ...
            if ref_frame != "world":
                ...
                abs_action_base_mat = self.env.robots[
                    0
                ].composite_controller.joint_action_policy.transform_pose(
                ...
                delta_action_base_mat = self.env.robots[
                    0
                ].composite_controller.joint_action_policy.transform_pose(
                ...
                delta_action = np.concatenate(
                    [
                        delta_action_base_mat[:3, 3],
                        T.quat2axisangle(T.mat2quat(delta_action_base_mat[:3, :3])),
                    ]
                )
                abs_action = np.concatenate(
                    [
                        abs_action_base_mat[:3, 3],
                        T.quat2axisangle(T.mat2quat(abs_action_base_mat[:3, :3])),
                    ]
                )
    ...
    def get_torso_action(self, robot, device_input):
        ...
        if torso_controller.name == "JOINT_POSITION":
            if torso_controller.input_type == "delta":
                ...
                scale = 0.01
                target = (
                    self._prev_torso_target
                    if self._prev_torso_target is not None
                    else torso_controller.goal_qpos
                )
