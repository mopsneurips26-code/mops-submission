...
class DemoSamplerWrapper(Wrapper):
    ...
    def __init__(
        ...
        assert env_name == self.unwrapped.__class__.__name__, (
            f"Wrapped env {env.__class__.__name__} does not match env on which demos were collected ({env_name})"
        )
    ...
    def _uniform_sample(self):
        ...
        states = self.demo_file[f"data/{ep_ind}/states"][()]
    ...
    def _xml_for_episode_index(self, ep_ind):
        ...
        model_file = self.demo_file[f"data/{ep_ind}"].attrs["model_file"]
        model_path = os.path.join(self.demo_path, "models", model_file)
        with open(model_path) as model_f:
