...
def update(
    self, q: np.ndarray | None = None, update_idxs: np.ndarray | None = None
) -> None:
...
def compute_translation_error(self, configuration: Configuration) -> np.ndarray:
    ...
    return np.linalg.norm(
        ...
        - transform_frame_to_world.translation()
    )
...
class IKSolverMink:
    def __init__(
        ...
        for i in range(self.robot_model.njnt):
            ...
            self.all_robot_qpos_indexes_in_full_model.extend(
                self.full_model.joint(joint_name).qposadr[0]
                + np.arange(len(self.full_model.joint(joint_name).qpos0))
            )
        ...
        for name in robot_joint_names:
            self.controlled_robot_qpos_indexes.extend(
                self.robot_model.joint(name).qposadr[0]
                + np.arange(len(self.robot_model.joint(name).qpos0))
            )
            self.controlled_robot_qpos_indexes_in_full_model.extend(
                self.full_model.joint(name).qposadr[0]
                + np.arange(len(self.full_model.joint(name).qpos0))
            )

        self.site_ids = [
            self.robot_model.site(site_name).id for site_name in site_names
        ]
    ...
    def _setup_tasks(self) -> None:
        ...
        if self.use_mink_posture_task:
            self.posture_task = mink.PostureTask(
                self.robot_model, cost=weights * 0.1, lm_damping=1.0
            )
        else:
            self.posture_task = WeightedPostureTask(
                self.robot_model, cost=0.01, weights=weights, lm_damping=2
            )
        ...
        self.hand_tasks = self._create_frame_tasks(
            self.site_names,
            position_cost=self.hand_pos_cost,
            orientation_cost=self.hand_ori_cost,
        )
    ...
    def _create_frame_tasks(
        self, frame_names: list[str], position_cost: float, orientation_cost: float
    ):
        return [
            mink.FrameTask(
                frame_name=frame,
    ...
    def update_robot_states(self) -> None:
        # update the base pose, important for mobile robots such as humanoids
        self.configuration.model.body("robot0_base").pos = self.full_model.body(
            "robot0_base"
        ).pos
        self.configuration.model.body("robot0_base").quat = self.full_model.body(
            "robot0_base"
        ).quat
    ...
    def set_target_poses(self, target_poses: list[np.ndarray]) -> None:
        for task, target in zip(self.hand_tasks, target_poses, strict=False):
            se3_target = mink.SE3.from_matrix(target)
            task.set_target(se3_target)

    def set_posture_target(self, posture_target: np.ndarray | None = None) -> None:
        if posture_target is None:
            self.posture_task.set_target_from_configuration(self.configuration)
        else:
            self.posture_task.set_target(posture_target)

    def set_com_target(self, com_target: np.ndarray | None = None) -> None:
    ...
    def transform_pose(
        ...
        self.robot_model.body("robot0_base").pos = self.full_model.body(
            "robot0_base"
        ).pos
        self.robot_model.body("robot0_base").quat = self.full_model.body(
            "robot0_base"
        ).quat
        ...
        if src_frame != "world":
            X_W_src_frame = self.configuration.get_transform_frame_to_world(
                src_frame, "body"
            ).as_matrix()
    ...
    def solve(self, input_action: np.ndarray) -> np.ndarray:
        ...
        if self.input_rotation_repr == "axis_angle":
            input_quat_wxyz = np.array(
                [
                    np.roll(T.axisangle2quat(input_ori[i]), 1)
        ...
        if self.input_ref_frame == "base":
            ...
            if self.input_type == "absolute":
                ...
                input_poses = []
                for pos, quat in zip(input_pos, input_quat_wxyz, strict=False):
                ...
                input_quat_wxyz = np.array(
                    [np.roll(T.mat2quat(pose[:3, :3]), 1) for pose in input_poses]
                )
        ...
        if "delta" in self.input_type:
            cur_pos = np.array(
                [
                    self.configuration.data.site(site_id).xpos
                    for site_id in self.site_ids
                ]
            )
            cur_ori = np.array(
                [
                    self.configuration.data.site(site_id).xmat
                    for site_id in self.site_ids
                ]
            )
        if self.input_type == "delta":
            ...
            target_poses = np.array(
                [
                    np.dot(cur_poses[i], delta_poses[i])
                    for i in range(len(self.site_ids))
                ]
            )
            ...
            target_quat_wxyz = np.array(
                [
                    np.roll(T.mat2quat(target_poses[i, :3, :3]), shift=1)
                    for i in range(len(self.site_ids))
                ]
            )
        ...
        for i, (pos, quat_wxyz) in enumerate(
            zip(target_pos, target_quat_wxyz, strict=False)
        ):
            targets[i][:3, 3] = pos
            targets[i][:3, :3] = T.quat2mat(np.roll(quat_wxyz, -1))

        # set target poses
        for task, target in zip(self.hand_tasks, targets, strict=False):
        ...
        with suppress_stdout():
            vel = mink.solve_ik(
                self.configuration, self.tasks, 1 / self.solve_freq, self.solver, 1e-5
            )
...
class WholeBodyMinkIK(WholeBody):
    ...
    def _validate_composite_controller_specific_config(self) -> None:
        ...
        ik_posture_weights = self.composite_controller_specific_config.get(
            "ik_posture_weights", {}
        )
        ...
        self.composite_controller_specific_config["ik_posture_weights"] = (
            valid_posture_weights
        )

    def _init_joint_action_policy(self) -> None:
        joint_names: str = []
        ...
        self.joint_action_policy = IKSolverMink(
            ...
            site_names=(
                self.composite_controller_specific_config.get(
                    "ref_name", default_site_names
                )
            ),
            ...
            solve_freq=self.composite_controller_specific_config.get(
                "ik_solve_freq", 20
            ),
            posture_weights=self.composite_controller_specific_config.get(
                "ik_posture_weights", {}
            ),
            hand_pos_cost=self.composite_controller_specific_config.get(
                "ik_hand_pos_cost", 1.0
            ),
            hand_ori_cost=self.composite_controller_specific_config.get(
                "ik_hand_ori_cost", 0.5
            ),
            use_mink_posture_task=self.composite_controller_specific_config.get(
                "use_mink_posture_task", False
            ),
