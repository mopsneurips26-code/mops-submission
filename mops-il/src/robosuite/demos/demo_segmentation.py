...
def randomize_colors(N, bright=True):
    ...
    colors = np.array([colorsys.hsv_to_rgb(*c) for c in hsv])
    np.random.RandomState(seed=20)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--env-name", type=str, default="TwoArmHandover", help="Environment name"
    )
    ...
    for i in range(100):
        ...
        video_img = obs[f"{args.camera}_segmentation_{segmentation_level}"].squeeze(-1)[
            ::-1
        ]
        np.savetxt(f"/tmp/seg_{i}.txt", video_img, fmt="%.2f")
        video_img = segmentation_to_rgb(video_img, args.random_colors)
        video_writer.append_data(video_img)

        image = Image.fromarray(video_img)
        image.save(f"/tmp/seg_{i}.png")
        if i % 5 == 0:
            print(f"Step #{i} / 100")
