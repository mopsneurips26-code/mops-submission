...
def register_kitchen_env(target_class) -> None:
...
class Kitchen(ManipulationEnv, metaclass=KitchenEnvMeta):
    ...
    def _load_model(self) -> None:
        ...
        if macros.VERBOSE:
            print(f"layout: {self.layout_id}, style: {self.style_id}")
        ...
        for i in range(10):
            try:
                ...
                if macros.VERBOSE:
                    print(f"Ranomization error in initial placement. Try #{i}")
                continue
            break
        ...
        for i in range(1):
            try:
                ...
                if macros.VERBOSE:
                    print(f"Randomization error in initial placement. Try #{i}")
                continue
            break
    ...
    def compute_robot_base_placement_pose(self, ref_fixture, offset=None):
        ...
        base_fixtures = [
            ...
            if isinstance(fxtr, (Counter, Stove, Stovetop, HousingCabinet, Fridge))
        ]
        ...
        if (
            isinstance(base_fixture, (HousingCabinet, Fridge))
    ...
    def _get_placement_initializer(self, cfg_list, z_offset=0.01):
        ...
        for obj_i, cfg in enumerate(cfg_list):
            ...
            if macros.SHOW_SITES is True:
                ...
                site_str = f"""<site type="box" rgba="0 0 1 0.4" size="{array_to_string(size_to_vis)}" pos="{array_to_string(pos_to_vis)}" name="reset_region_outer_{str(obj_i)}"/>"""
                ...
                site_str = f"""<site type="box" rgba="1 0 0 0.4" size="{array_to_string(size_to_vis)}" pos="{array_to_string(pos_to_vis)}" name="reset_region_inner_{str(obj_i)}"/>"""
                site_tree = ET.fromstring(site_str)
                self.model.worldbody.append(site_tree)
    ...
    def _reset_internal(self) -> None:
        ...
        for _i in range(10 * int(self.control_timestep / self.model_timestep)):
            self.sim.step1()
    ...
    def get_ep_meta(self):
        ...
        ep_meta["lang"] = ""
        ep_meta["fixture_refs"] = {k: v.name for (k, v) in self.fixture_refs.items()}
    ...
    def _randomize_cameras(self) -> None:
        ...
        for camera in self._cam_configs:
            ...
            new_pos = [pos + n for pos, n in zip(old_pos, pos_noise, strict=False)]
            ...
            new_euler = [
                eul + n for eul, n in zip(old_euler, euler_noise, strict=False)
            ]
    ...
    def _is_fxtr_valid(self, fxtr, size) -> bool:
    ...
    def get_fixture(self, id, ref=None, size=(0.2, 0.2)):
        ...
        if ref is None:
            ...
            if isinstance(id, (FixtureType, int)):
                ...
                matches = [name for name in self.fixtures if id in name]
            ...
            close_fixtures = [
                fxtr
                for (fxtr, d) in zip(cand_fixtures, dists, strict=False)
                if d - min_dist < 0.10
            ]
