...
class BeverageSorting(Kitchen):
    ...
    def _setup_kitchen_references(self) -> None:
        ...
        self.cab1 = self.register_fixture_ref(
            "cabinet1", {"id": FixtureType.CABINET_TOP}
        )
        self.cab2 = self.register_fixture_ref(
            "cabinet2", {"id": FixtureType.CABINET_TOP, "ref": self.cab1}
        )
        self.counter = self.register_fixture_ref(
            "counter", {"id": FixtureType.COUNTER, "size": (0.5, 0.5), "ref": self.cab1}
        )
    ...
    def _check_success(self) -> bool | None:
        ...
        for c1, c2 in [(self.cab1, self.cab2), (self.cab2, self.cab1)]:
            if (
                OU.obj_inside_of(self, "alcohol1", c1)
                and OU.obj_inside_of(self, "alcohol2", c1)
                and OU.obj_inside_of(self, "non_alcohol1", c2)
                and OU.obj_inside_of(self, "non_alcohol2", c2)
            ):
                return True
