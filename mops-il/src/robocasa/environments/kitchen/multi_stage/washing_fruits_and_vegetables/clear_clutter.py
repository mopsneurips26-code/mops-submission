...
class ClearClutter(Kitchen):
    ...
    def _setup_kitchen_references(self) -> None:
        ...
        self.counter = self.register_fixture_ref(
            "counter", {"id": FixtureType.COUNTER, "ref": self.sink, "size": (0.6, 0.6)}
        )
    ...
    def _check_success(self):
        food_in_sink = all(
            OU.obj_inside_of(self, f"obj_{i}", self.sink) for i in range(self.num_food)
        )
        ...
        unwashables_not_in_tray = all(
            not OU.check_obj_in_receptacle(self, f"unwashable_obj_{i}", "receptacle")
            for i in range(self.num_unwashable)
        )
