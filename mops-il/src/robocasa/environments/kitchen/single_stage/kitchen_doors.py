...
class ManipulateDoor(Kitchen):
    ...
    def _setup_kitchen_references(self) -> None:
        ...
        self.door_fxtr = self.register_fixture_ref("door_fxtr", {"id": self.door_id})
        ...
        cfgs.append(
            {
                ...
                "graspable": True,
                "microwavable": (
                    True if isinstance(self.door_fxtr, Microwave) else None
                ),
                "placement": {
...
class OpenDoubleDoor(OpenDoor):
    def __init__(
        self, door_id=FixtureType.DOOR_TOP_HINGE_DOUBLE, *args, **kwargs
    ) -> None:
        super().__init__(door_id=door_id, *args, **kwargs)


class CloseDoor(ManipulateDoor):
    def __init__(self, behavior=None, *args, **kwargs) -> None:
