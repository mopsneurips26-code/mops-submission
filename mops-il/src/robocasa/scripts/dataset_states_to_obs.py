...
def write_traj_to_file(
    ...
    print(f"total processes end {total_run.value}")
    data_grp.attrs["env_args"] = json.dumps(
        env.serialize(), indent=4
    )  # environment info
    print(f"Wrote {total_samples.value} total samples to {output_path}")
...
def extract_multiple_trajectories(
    ...
    try:
        ...
        print("*>*" * 50)
        print(f"Error process num {process_num}:")
        print(e)
...
def extract_multiple_trajectories_with_error(
    ...
    while (not work_queue.empty()) and (ind != -1):
        try:
            ...
            print(f"Error processing demo index {ind}: {e}")
            print(traceback.format_exc())
            print("_" * 50)
            del env
...
def dataset_states_to_obs_multiprocessing(args) -> None:
    # create environment to use for data processing
    ...
    DatasetUtils.get_env_metadata_from_dataset(dataset_path=args.dataset)
