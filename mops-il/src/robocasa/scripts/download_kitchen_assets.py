...
def url_is_alive(url) -> bool | None:
...
def download_url(url, download_dir, fname=None, check_overwrite=True) -> None:
...
def download_and_extract_zip(
    ...
    download_path = os.path.join(download_dir, f"{os.path.basename(folder)}.zip")

    print(colored(message, "yellow"))

    # extract files
    if check_folder_exists and os.path.exists(folder):
        ans = input(f"{folder} already exists! \noverwrite? (y/n) ")
    ...
    for i in range(3):
        try:
            ...
            print(f"Error downloading after try #{i + 1}")
...
def download_kitchen_assets() -> None:
