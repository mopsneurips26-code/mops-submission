...
def log_info(message, color="yellow") -> None:
...
if __name__ == "__main__":
    ...
    for ep in range(10):
        ...
        print(f"ep #{ep + 1}")
        print(f"   {reset_time:.2f}s reset time")
        print(f"   {steps_per_sec:.2f} fps")
        print()
        reset_time_list.append(reset_time)
        steps_per_sec_list.append(steps_per_sec)

    print(f"reset time: {np.mean(reset_time_list):.2f}s")
    print(f"fps: {np.mean(steps_per_sec_list):.2f}")
