            ...
            if i < traj_len - 1:
                ...
                if not np.all(np.equal(states[i + 1], state_playback)):
                    ...
                    if verbose or i == traj_len - 2:
                        print(
                            colored(
                                f"warning: playback diverged by {err} at step {i}",
                                "yellow",
                            )
                        )
        else:
...
def playback_dataset(args) -> None:
    # some arg checking
    ...
    for ind in range(len(demos)):
        ep = demos[ind]
        print(colored(f"\nPlaying back episode: {ep}", "yellow"))
        ...
        if args.use_actions:
            ...
            actions = f[f"data/{ep}/actions_abs"][()]  # absolute actions
