...
class Microwave(Fixture):
    ...
    def handle_name(self) -> str:
        return f"{self.name}_door_handle"
    ...
    def update_state(self, env) -> None:
        ...
        start_button_pressed = env.check_contact(
            env.robots[0].gripper["right"], f"{self.name}_start_button"
        )
        stop_button_pressed = env.check_contact(
            env.robots[0].gripper["right"], f"{self.name}_stop_button"
        )
    ...
    def gripper_button_far(self, env, button, th=0.15):
        ...
        button_id = env.sim.model.geom_name2id(f"{self.naming_prefix}{button}")
