...
class Accessory(Fixture):
    ...
    def __init__(self, xml, name, pos=None, *args, **kwargs) -> None:
        super().__init__(
            xml=xml,
            name=name,
...
class CoffeeMachine(Accessory):
    ...
    def __init__(self, *args, **kwargs) -> None:
        ...
        for postfix in ["coffee_liquid_left", "coffee_liquid_right", "coffee_liquid"]:
            name = f"{self.naming_prefix}{postfix}"
            site = self.worldbody.find(f"./body/body/site[@name='{name}']")
            if site is not None:
...
class WallAccessory(Fixture):
    ...
    def _place_accessory(self) -> None:
