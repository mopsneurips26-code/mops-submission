...
from robosuite.utils.mjcf_utils import find_elements, new_geom, xml_path_completion
...
class Counter(ProcGenFixture):
    ...
    def __init__(
        ...
        top_texture=None,
        top_thickness=0.03,
        half_top=None,  # for aligning corner counters
        # base, can use both
        base_texture=None,
        base_color=None,
        base_opening=None,
    ...
    def _set_texture(self, top_texture, base_texture, base_color) -> None:
        ...
        material = find_elements(
            self.root,
            tags="material",
            attribs={"name": f"{self.name}_counter_top"},
            return_first=True,
        )
        ...
        material = find_elements(
            self.root,
            tags="material",
            attribs={"name": f"{self.name}_counter_base"},
            return_first=True,
        )
        material.set("texture", tex_name)
        ...
        for geom_name in geoms:
            for postfix in ["", "_visual"]:
                g = find_elements(
                    root=self._obj,
                    tags="geom",
                    attribs={"name": f"{self.name}_{geom_name}{postfix}"},
                    return_first=True,
                )
    ...
    def _make_counter(self) -> None:
        ...
        for i in range(len(chunk_sizes)):
            ...
            self._contact_geoms.append(f"top_{i}")

        base_size, base_pos = self._get_base_dimensions()

        for side in SIDES:
            for elem in geoms[f"base_{side}"]:
                ...
                if (
                    ...
                    or side == "back"
                    and self.hollow[0]
                    or sum(self.base_opening) == 0
                    and sum(self.hollow) > 0
                ):
    ...
    def _make_counter_with_opening(self, padding) -> None:
        ...
        if front_pad < self.overhang:
            raise ValueError(
                ...
                f"than front padding ({front_pad:.2f})"
            )
        ...
        for side in SIDES:
            ...
            geom_name = self._name + f"_top_{side}"
            ...
            self._obj.append(g_vis)
            # manually update visual geoms registry
            self._visual_geoms.append(f"top_{side}_visual")
            ...
            for i in range(len(chunk_sizes)):
                ...
                self._obj.append(g)
                self._contact_geoms.append(f"top_{side}_{i}")
    ...
    def get_reset_regions(self, env, ref=None, loc="nn", top_size=(0.4, 0.4)):
        ...
        if ref is None:
            ...
            for g in all_geoms:
                ...
                reset_regions[f"geom_{geom_i}"] = {"size": size, "offset": offset}
                geom_i += 1
        else:
            ...
            if loc == "nn":
                ...
                for offset, g in zip(fixture_to_geom_offsets, all_geoms, strict=False):
                    if offset[0] > 0.30:
