...
class Stove(Fixture):
    ...
    def __init__(
        self, xml="fixtures/stoves/stove_orig", name="stove", *args, **kwargs
    ) -> None:
    ...
    def get_reset_regions(self, env, locs=None):
        ...
        for location in locs:
            site = self.worldbody.find(
                f"./body/body/site[@name='{self.naming_prefix}burner_{location}_place_site']"
            )
            if site is None:
                site = self.worldbody.find(
                    f"./body/body/site[@name='{self.naming_prefix}burner_on_{location}']"
                )
            if site is None:
                continue
    ...
    def update_state(self, env) -> None:
        ...
        for location in STOVE_LOCATIONS:
            ...
            site_id = env.sim.model.site_name2id(
                f"{self.naming_prefix}burner_on_{location}"
            )
            ...
            joint_id = env.sim.model.joint_name2id(
                f"{self.naming_prefix}knob_{location}_joint"
            )
    ...
    def set_knob_state(self, env, rng, knob, mode="on") -> None:
        ...
        env.sim.data.set_joint_qpos(f"{self.naming_prefix}knob_{knob}_joint", joint_val)
        ...
        if self._knob_joints is None:
            ...
            for location in STOVE_LOCATIONS:
                joint = self.worldbody.find(
                    f"./body/body/body/joint[@name='{self.naming_prefix}knob_{location}_joint']"
                )
...
class Oven(Fixture):
    ...
    def __init__(
        self, xml="fixtures/ovens/samsung", name="oven", *args, **kwargs
    ) -> None:
        super().__init__(
            xml=xml, name=name, duplicate_collision_geoms=False, *args, **kwargs
        )
