...
class SingleCabinet(Cabinet):
    ...
    def handle_name(self) -> str:
        return f"{self.name}_door_handle_handle"

    @property
    def door_name(self) -> str:
        return f"{self.name}_hingedoor"
...
class HingeCabinet(Cabinet):
    ...
    def _create_cab(self) -> None:
        ...
        for side in ["left", "right"]:
            self._add_door(
                ...
                parent_body=bodies[f"hinge{side}door"],
    ...
    def set_door_state(self, min, max, env, rng) -> None:
        ...
        env.sim.data.set_joint_qpos(
            f"{self.name}_rightdoorhinge", rng.uniform(desired_min, desired_max)
        )

        env.sim.data.set_joint_qpos(
            f"{self.name}_leftdoorhinge", -rng.uniform(desired_min, desired_max)
        )
    ...
    @property
    def left_handle_name(self) -> str:
        return f"{self.name}_left_door_handle_handle"

    @property
    def right_handle_name(self) -> str:
        return f"{self.name}_right_door_handle_handle"
...
class HousingCabinet(Cabinet):
    ...
    def _place_interior_obj(self) -> None:
