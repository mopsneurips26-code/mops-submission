...
class Sink(Fixture):
    ...
    def __init__(self, xml="fixtures/sink.xml", name="sink", *args, **kwargs) -> None:
    ...
    def update_state(self, env) -> None:
        ...
        site_id = env.sim.model.site_name2id(f"{self.naming_prefix}water")
    ...
    def set_handle_state(self, env, rng, mode="on") -> None:
        ...
        env.sim.data.set_joint_qpos(f"{self.naming_prefix}handle_joint", joint_val)
    ...
    def handle_joint(self):
        ...
        if self._handle_joint is None:
            self._handle_joint = self.worldbody.find(
                f"./body/body/body/joint[@name='{self.naming_prefix}handle_joint']"
            )
    ...
    def water_site(self):
        ...
        if self._water_site is None:
            self._water_site = self.worldbody.find(
                f"./body/body/body/site[@name='{self.naming_prefix}water']"
            )
