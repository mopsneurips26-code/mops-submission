    ...
    if show_bbox:
        ...
        if (ext_bbox_center is not None) and (ext_bbox_size is not None):
            ext_bbox_site = ET.fromstring(
                f"""<site type="box" pos="{a2s(ext_bbox_center)}" size="{a2s(ext_bbox_size / 2)}" rgba="0 1 0 0.2"/>"""
            )
...
if __name__ == "__main__":
    ...
    if "aigen" in obj_registries:
        ...
        if os.path.exists(aigen_objs_path) is False:
            ...
            print(
                colored(
                    "Ending script. Rerun script to use new AI-generated objects",
                    "yellow",
                )
            )
            exit()
