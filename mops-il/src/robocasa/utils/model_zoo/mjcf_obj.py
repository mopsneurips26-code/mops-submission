...
class MJCFObject(MujocoXMLObject):
    ...
    def __init__(
        ...
        for n in ["bottom_site", "top_site", "horizontal_radius_site"]:
            site = root.find(f"worldbody/body/site[@name='{n}']")
        ...
        new_xml_path = os.path.join(folder, f"{time_str}_{os.getpid()}.xml")
    ...
    def _get_geoms(self, root, _parent=None):
        ...
        """
        geom_pairs = super()._get_geoms(root=root, _parent=_parent)

        # modify geoms according to the attributes
        for _i, (_parent, element) in enumerate(geom_pairs):
    ...
    def get_bbox_points(self, trans=None, rot=None):
        ...
        rot = T.quat2mat(rot) if rot is not None else np.eye(3)
