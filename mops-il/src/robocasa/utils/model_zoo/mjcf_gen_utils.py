...
def parse_model(
    ...
    LogUtils.maybe_log_info(
        f"parsing (main) model: {model_path}",
        log=verbose,
        indent=True,
    )
    ...
    if prescale:
        ...
        transform = mat_s if transform is None else np.matmul(mat_s, transform)
    ...
    for r in rot:
        ...
        if r in ["x", "x90"]:
            ...
            raise ValueError(f"Invalid choice of rotation {r}")

        transform = mat_R if transform is None else np.matmul(mat_R, transform)
    ...
    for sm_i, (sm_name, sm) in enumerate(submodels):
        ...
        vis_geom_path = os.path.join(vis_path, f"{obj_name}_{sm_i}.obj")
        # shutil.copyfile(
        #     obj_file,
        #     vis_geom_path,
        # )
        ...
        coll_geom_path = os.path.join(coll_path, f"{obj_name}_{sm_i}.obj")
        coll_geom = dict(
            obj_type="collision",
            geom_type="mesh",
            # path=os.path.join(coll_path, "{}_ch.stl".format(model_name)),
            path=coll_geom_path,
            name=f"{obj_name}_{sm_i}_coll",
...
def parse_coll_model(
    ...
    LogUtils.maybe_log_info(
        f"parsing collision model: {coll_model_path}",
        log=verbose,
        indent=True,
    )
...
def get_geom_element(model_name, geom_info, sc=1.0, show_if_coll_geom=False):
    ...
    if geom_info["geom_type"] == "mesh":
        ...
        elem.attrib["pos"] = " ".join([f"{v * sc:.5f}" for v in geom_info["pos"]])
        elem.attrib["quat"] = " ".join([f"{v:.5f}" for v in geom_info["quat"]])
        size = geom_info["size"]
        elem.attrib["size"] = " ".join([f"{v * sc / 2:.5f}" for v in size])
...
def generate_mjcf(
    ...
    LogUtils.maybe_log_info(f"Wrote MJCF to: {mjcf_path}", log=verbose, indent=True)
