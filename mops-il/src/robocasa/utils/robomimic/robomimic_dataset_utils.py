...
def extract_action_dict(dataset) -> None:
    # find files
    ...
    for spec in SPECS:
        ...
        prefix = "abs_" if is_absolute else "rel_"

        for demo in f["data"].values():
            if str(input_action_key) not in demo:
                continue
...
def filter_dataset_size(
    ...
    if input_filter_key is not None:
        ...
        demos = sorted(
            [elem.decode("utf-8") for elem in np.array(f[f"mask/{input_filter_key}"])]
        )
    else:
        demos = sorted(f["data"].keys())
    f.close()
    ...
    create_hdf5_filter_key(hdf5_path=hdf5_path, demo_keys=subset_keys, key_name=name)


def move_demo_to_new_key(f, old_demo_key, new_demo_key, delete_old_demo=True) -> None:
...
def make_demo_ids_contiguous(dataset) -> None:
    ...
    num_old_demos - len(missing_demo_inds)
...
def convert_to_robomimic_format(
    ...
    for ep in f["data"]:
        ...
        assert "model_file" in f[f"data/{ep}"].attrs

        # add "num_samples" into per-episode metadata
        if "num_samples" in f[f"data/{ep}"].attrs:
            del f[f"data/{ep}"].attrs["num_samples"]
        n_sample = f[f"data/{ep}/actions"].shape[0]
        f[f"data/{ep}"].attrs["num_samples"] = n_sample
