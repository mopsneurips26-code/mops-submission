...
def set_geom_dimensions(sizes, positions, geoms, rotated=False) -> None:
...
def obj_in_region(
    ...
    if isinstance(obj, (MJCFObject, Fixture)):
...
def objs_intersect(
    ...
    bbox_check = (isinstance(obj, (MJCFObject, Fixture))) and (
        isinstance(other_obj, (MJCFObject, Fixture))
    )
    if bbox_check:
        obj_points = obj.get_bbox_points(trans=obj_pos, rot=obj_quat)
        ...
        intersect = bool(xy_collision and z_collision)
