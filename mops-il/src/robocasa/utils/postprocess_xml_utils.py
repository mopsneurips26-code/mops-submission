...
def convert_xml(xml_str) -> None:
    ...
    for body in worldbody.iter("body"):
        ...
        if "link0" in body_name and body not in bodies_to_process:
    ...
    for asset in assets.iter("mesh"):
        path = asset.get("file")
        assert "robosuite/models/assets" in path or "robocasa/models/assets" in path, (
            print(path)
        )
        if "mounts" in path:
